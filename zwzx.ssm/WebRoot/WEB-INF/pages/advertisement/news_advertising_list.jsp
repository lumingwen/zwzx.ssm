...
<script type="text/javascript">
	...
	function add() {
		var url = 'managerAdvertising.do';
		location = url;
	}
	
	function edit(id) {
		var url = 'editAdvertising.do?bid=' + (id ? id : '');
		location = url;
	}
	
	function del(id){
    		confirms("确认删除","deleteAdvertisingInfo.do?id=" + ( id ? id : ''));
    	}
    	
	function view(id){
		   var url ='findAdvertisingById.do?bid=' + (id ? id : '');
			location=url;
    }
...
<body onload="init()">
	...
	<form action="listAdvertisingInfo.do" name="myForm" id="myForm" method="post">
	<div id="page-content" class="clearfix">
			...
			<div class="page-header position-relative">
				广告名称：<input type="text" name="adName" class="small2" value="${queryParam.adName}" />
				类型：
                   <select name="category">
					<option value=""
						<c:if test="${''==queryParam.category}">selected="selected"</c:if>>全部</option>

					<option value="2"
						<c:if test="${'text'==queryParam.category}">selected="selected"</c:if>>文字</option>
					<option value="1"
						<c:if test="${'image'==queryParam.category}">selected="selected"</c:if>>图片</option>
				</select>
		...
		<c:forEach items="${list}" var="requirement">
                        <tr>
							<td>${requirement.advertisingId}</td>
							...
							<c:if test="${requirement.category=='image'}">
							<td>图片</td>			
							</c:if>
							<c:if test="${requirement.category=='text'}">
							<td>文字</td>			
							</c:if>	
							...
							<td class="handle">
	                        <span class="iconfont show" onclick="view(${requirement.advertisingId});"><i class="fa fa-eye"></i></span>                   
	                    	<span class="iconfont update" onclick="edit(${requirement.advertisingId});"><i class="fa fa-edit"></i></span>
	                    	<span class="iconfont deletes" onclick="del(${requirement.advertisingId});"><i class="fa fa-trash-o"></i></span>	              
	                        </td>
					  </tr>
       </c:forEach>
</tbody>
