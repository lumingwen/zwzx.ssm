...
Cms.swfUpload = function(name, settings) {
	...
	var def_settings = {
		...
		flash9_url: "${pageContext.request.contextPath}/js/swfupload/swfupload_fp9.swf",
		file_post_name: "file",
...
Cms.swfUploadImage = function(name, settings) {
	...
	settings.upload_url = settings.upload_url || "/resAttachmentController/addEditorImage.do";
...
Cms.swfUploadImages = function(name, settings, addImageRow) {
	...
	settings.button_action = SWFUpload.BUTTON_ACTION.SELECT_FILES;
	settings.file_types_description = "Images";
	settings.upload_url = settings.upload_url || "${pageContext.request.contextPath}/resAttachmentController/ajaxUpload.do";
	settings.upload_start_handler = function(file) {
	    var params={};
	    if($("#s_"+name).length!=0) {
	    	params.scale=$("#s_"+name).prop("checked");
