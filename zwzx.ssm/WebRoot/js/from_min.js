	...
	a.util = {
		...
		enhance : function(l, m, n, k) {
			...
			o.find("[datatype]").each(function() {
				if (l == 2) {
					if (d(this).parent().next().find(".message").length == 0) {
						d(this).parent().next().append("<span class='message'/>");
						d(this).siblings(".message").remove()
					}
				} else {
					if (l == 3 || l == 4) {
						if (d(this).siblings(".message").length == 0) {
							//alert(d(this).parent().parent().html())
							d(this).parent().append("<span class='message text-error' />");
							d(this).parent().next().find(".message").remove()
						}
					}
				}
			});
			...
			if (v) {
				...
				if (u.attr("recheck")) {
					...
					if (n != p.val()) {
						v = false;
						l = 3;
						y = u.attr("alt") || a.util.getErrormsg.call(u, A, t, "recheck")
					}
				}
			} else {
				y = y || u.attr("alt") || a.util.getErrormsg.call(u, A, t);
		...
		showmsg : function(n, l, m, k) {
			...
			if (k == "bycheck" && m.sweep && (m.obj && !m.obj.is(".error") || typeof l == "function")) {
				return
			}
			d.extend(m, {
				curform : this
			...
			if (l == 2 && m.obj) {
				m.obj.parent().next().find(".message").html(n);
				a.util.cssctl(m.obj.parent().parent().next().find(".message"), m.type)
			}
			if ((l == 3 || l == 4) && m.obj) {
				m.obj.nextAll(".message").eq(0).html(n);
				a.util.cssctl(m.obj.nextAll(".message").eq(0), m.type)
			}
		},
		cssctl : function(l, k) {
			switch(k) {
				case 1:
					l.removeClass("Validform_right Validform_wrong").addClass("message Validform_loading");
					break;
				case 2:
					l.removeClass("Validform_wrong Validform_loading").addClass("message Validform_right");
					break;
				case 4:
					l.removeClass("Validform_right Validform_wrong Validform_loading").addClass("message");
					break;
				default:
					l.removeClass("Validform_right Validform_loading").addClass("message Validform_wrong")
			}
		},
		...
		resetForm : function() {
			...
			k.find(".message").removeClass("Validform_wrong Validform_right Validform_loading");
			k.find(".error").removeClass("error");
	...
	a.prototype = {
		...
		ignore : function(k) {
			...
			d(l.forms).find(k).each(function() {
				d(this).data("dataIgnore", "dataIgnore").removeClass("error")
			});
			return this
		},
		unignore : function(k) {
...
//复选框
$.fn.ruleSingleCheckbox = function (stat,end) {
...
$.fn.ruleSingleSelect = function () {
    var singleSelect = function (parentObj) {
        ...
        parentObj.find("select").hide(); //隐藏内容
        var divObj = $('<div class="boxwrap"></div>').prependTo(parentObj); //前插入一个DIV
        //创建元素
...
$("#content_edit").Validform({
	datatype : {
		"phone" : function(gets, obj, curform, regxp) {
			var reg1 = regxp["m"], reg2 = /[\d]{7}/, mobile = curform.find(".mobile");
