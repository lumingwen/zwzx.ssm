...
$(function() {
	...
	$.fn.InitSWFUpload = function(p) {
		...
		parentObj.append('<div class="upload-btn"><span id="' + parentBtnId + '"></span></div><label>图片限2M以下，张数4张以内，格式jpg、jpeg、png、bmp;</label>');
		//初始化属性
		...
		//初始化上传控件
		swfu = new SWFUpload({
			post_params: (p && p.post_params) ? p.post_params : {
				"ASPSESSID": "NONE"
			},
			use_query_string: (p && p.use_query_string) ? p.use_query_string : true,
			upload_url: p.sendurl, //上传地址
			file_size_limit: p.filesize, //文件大小
			file_types: p.filetypes, //文件类型
			file_types_description: "JPG Images",
			file_upload_limit: "12", //一次能上传的文件数量
			...
			upload_success_handler: p.uploadSuccess ? p.uploadSuccess : uploadSuccess,
			...
			button_text_style: ".btnText{font-family:Microsoft YaHei;display:block;font-size:14px;color:#ffffff;text-align:center;}", //按钮样式
...
function addImage(targetObj, originalSrc, thumbSrc) {
		...
		//  + '</li>');
		var newLi = $('<li>' + '<input type="hidden" value="0|' + thumbSrc + '" name="hid_photo_name">' + '<div onclick="setFocusImg(this);" class="img-box">' + '<img src="' + thumbSrc + '"><span  onclick="delImg(this);" class="remark"></span>' + '</div><a onclick="setFocusImg($(this).siblings(div));" href="javascript:;">设为主图</a>' + '</li>');
...
function loaded() {
	if ($("#imageUl>li").length > 0) {
		$("#imageUl>li").each(function(i) {
			var stats = swfu.getStats();
			stats.successful_uploads++;
			swfu.setStats(stats);
		});
